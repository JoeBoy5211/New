...
import { useApprovedCaterers } from '@/hooks/supabase/usePublicCaterers';
import type { Caterer as SupabaseCaterer } from '@/hooks/supabase/useCaterers';
import type { Caterer } from '@/data/mockData';
...
export default function BrowseCaterers() {
  ...
  const { data: approvedCaterers, isLoading } = useApprovedCaterers();

  // Map Supabase snake_case rows to the card's camelCase shape
  const allCaterers: Caterer[] = (approvedCaterers ?? []).map((c: SupabaseCaterer) => ({
    id: c.id,
    name: c.name,
    ...
    logoUrl: (c as Partial<SupabaseCaterer>).logo_url ?? null,
