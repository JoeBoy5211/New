...
export function useUpdateCaterer() {
  ...
  return useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<Caterer> }) => {
      const { data, error } = await supabase.from('caterers').update(updates).eq('id', id).select().maybeSingle();
      if (error) {
      ...
      if (!data) {
        throw new Error('Update returned no rows. RLS likely blocked the vendor update.');
      }
      return data;
    },
